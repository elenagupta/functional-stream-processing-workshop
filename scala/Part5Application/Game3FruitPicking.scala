...
object Game3FruitPicking
    extends GameAppWithErrorAndLoggingAndMetrics[
      Game3FruitPicking.GameState,
      Game3FruitPicking.Command
    ] {
    ...
    def simulation(state: Ref[IO, GameState]): Stream[IO, Nothing] =
      Stream
        ...
        .flatMap { random =>
          Stream
            .repeatEval(
              (random.nextIntBounded(50), random.nextIntBounded(50)).tupled
                .map((x, y) => Position(x, y))
                .flatMap { position =>
                  state.update(GameState.growFruit(_, position))
                }
            )
            .metered(1.second)
        }
